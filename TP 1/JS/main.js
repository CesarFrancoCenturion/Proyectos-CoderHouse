...
function calcularDescuento(total, porcentajeDescuento) {
    ...
    const montoDescuento = total * (porcentajeDescuento / 100)
    const precioConDescuento = total - montoDescuento
    console.log(`descuento aplicado ${porcentajeDescuento}%, el monto de descuento es: ${montoDescuento}, el precio con descuento final es: ${precioConDescuento}`)
...
switch(menu){
    ...
    case 4:
        if (precioFinal === 0){
                alert("para aplicar el descuento debe realizar una compra")
                break
        }
        let porcentaje = parseInt(prompt("ingresar el descuento que quiere aplicar"))
        let porcentajeDescuento = parseInt(porcentaje)

        //verificacion de si el porcentaje es valido    
        if (isNaN(porcentaje)){
            alert("porcentaje de descuento invalido. ingresar un nueno porcentaje")
            console.error(`error: ingreso un porcentaje de descuento no numerico ${porcentaje}`)
            break
        }
        //aca se aplica el descuento
        let precioConDescuento = calcularDescuento(precioFinal, porcentajeDescuento)
        alert(`el descuento aplicado es de: ${porcentajeDescuento}`)
